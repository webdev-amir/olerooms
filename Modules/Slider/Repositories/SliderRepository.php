...
class SliderRepository implements SliderRepositoryInterface {
            ...
            return DataTables::of($lists)
               ->addColumn('action', function($list) use($model){
                ...
                ->editColumn('url', function($list){
                    if($list->url){
                        return \Illuminate\Support\Str::limit($list->url, 100, '');    
                    }else{
                        return 'N/A';
                    }
    ...
    {
        try {
            $filleable = $request->only( 'slug','title','url','description','slider_order');
    ...
    {
        try {
            $filleable = $request->only( 'slug','title','url','description','status','slider_order');
            if ($request->get('banner_image')) $filleable['banner_image'] = $request->get('banner_image');
    ...
    {
         $filename = uploadWithResize($request->file('files'),'/slider/');
