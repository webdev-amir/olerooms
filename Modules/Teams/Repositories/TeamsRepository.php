...
class TeamsRepository implements TeamsRepositoryInterface
{
        ...
        try {
            ...
            return DataTables::of($lists)
                ...
                ->editColumn('type', function ($list) {
                    return \Illuminate\Support\Str::limit($list->team_type, 100, '');
                })
    ...
    {
        try {
            $filleable = $request->only('slug', 'name', 'team_type','order_number','designation','linkedin_url','description');
            if ($request->get('image')) $filleable['image'] = $request->get('image');
            $this->Teams->fill($filleable);
            $this->Teams->save();
            $response['message'] = trans('flash.success.teams_created_successfully');
    ...
    public function update($request, $id)
    {
        try {
            ...
            $record->save();
            $response['message'] = trans('flash.success.teams_updated_successfully');
            $response['type'] = 'success';
        } catch (Exception $ex) {
    ...
    public function changeStatus($request, $slug)
    {
        ...
        if ($Amenity) {
            $id = $Amenity->id;
            $change = $this->Teams->find($id);
            $active = $change->status;
            if ($id != null) {
                if ($active == 1) {
                    $update_arr = array('status' => 0);
                    $this->Teams->where('id', $id)->update($update_arr);
                } else {
                    $update_arr = array('status' => 1);
                    $this->Teams->where('id', $id)
                        ->update($update_arr);
                }
    ...
    {
        $filename = uploadWithResize($request->file('files'),'teams/');
