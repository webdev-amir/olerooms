...
class MyScheduleVisitCompanyRepository implements MyScheduleVisitRepositoryInterface {
    ...
    public function storeScheduleVisit($request) {
        ...
        if (!auth()->user()->hasRole('company')) {
            $response['message'] = 'Please login with company';
            $response['type'] = 'error';
            return $response;
        } else {
            ...
            if ($checkRec) {
                ...
                if (!empty($checkRec->payment_id) && $checkRec->payment_id != null) {
                    ...
                    $response['redirect-url'] = route('company.schedulevisit.success', $checkRec->slug);
                    return $response;
                } else if ($check == 0) {
                    ...
                    $response['store-visit'] = 1;
                    $response['redirect-url'] = route('company.schedulevisit.index', $checkRec->slug);
                    return $response;
                } else if ($check < 3) {
                    ...
                    if (empty($chkprop)) {
                        ...
                        $response['store-visit'] = 1;
                        $response['redirect-url'] = route('company.schedulevisit.index', $checkRec->slug);
                        // $response['redirect-url'] = route('company.search') . '?property_type=2';
                        return $response;
                    }
                } else {
                ...
                $response['type'] = 'success';
                $response['store-visit'] = 1;
                $response['redirect-url'] = route('company.schedulevisit.index', $add->slug);
                return $response;
            }
        }
    }
    ...
    public function updateScheduleVisit($request) {
        ...
        if ($scheduleVisit->TotalProperty == 0) {
            ...
            $response['type'] = 'error';
            $response['store-visit'] = 1;
            $response['redirect-url'] = route('company.schedulevisit.index', $scheduleVisit->slug);
            return $response;
        } else {
            if (setting_item('schedule_visit_amount') == 0 || setting_item('schedule_visit_amount') == NULL) {
                $response['message'] = 'Please contact to company support schedule visit charges not set by admin';
                $response['type'] = 'error';
                return $response;
            }
            ...
            if (count($request->get('visit')) > 3) {
                $response['message'] = 'You cannot add more then three property in single schedule';
                $response['type'] = 'error';
                return $response;
            }
