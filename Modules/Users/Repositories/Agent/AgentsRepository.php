...
class AgentsRepository implements AgentsRepositoryInterface
{
    ...
    {
        try {
            $role = $this->Role->where('slug', 'agent')->first();
            ...
            if ($user = $this->User->create($filleable)) {
                $user->agent_code = createAgentCorpCode($user->name, $user->id);
                $user->save();
                User::$guard_name = 'web';
                if ($role) {
                    $user->assignRole([$role->id]);
                    $this->EmailNotificationsRepo->sendWelcomeEmailToAgent($user, $password);
                }
            }
